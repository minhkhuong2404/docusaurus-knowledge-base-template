...
* Mỗi request đi qua 8 Microservices &rarr; sinh ra trung bình 8 Spans.
...
* Tốc độ sinh log tracing thô:
  ...
  - **Dữ liệu mỗi ngày:** 40 MB/s × 86,400 s &asymp; **3.45 TB/ngày**
  - **Dữ liệu mỗi tháng:** &asymp; **103.5 TB/tháng**
...
* Với chi phí APM thương mại trung bình **$0.10 - $0.20 / GB ingested**: Hóa đơn hàng tháng sẽ rơi vào khoảng **$10,000 - $20,000 USD** chỉ riêng cho tracing!
...
* **Nhược điểm chí mạng:**
  - **Mù hoàn toàn về kết quả tương lai (Blind to Future Failures):** Tại thời điểm request vừa tới API Gateway, hệ thống **không thể biết trước** liệu request này 200ms sau đó có bị lỗi `HTTP 500` ở Payment Service, hay bị timeout 10s ở Database query hay không.
  - **Quy luật xác suất bỏ sót sự cố:** Nếu bạn lấy mẫu 1% ($s = 0.01$), và hệ thống đang xảy ra lỗi bất thường với tỉ lệ 0.1% traffic, xác suất để bạn bắt được một request lỗi cụ thể chỉ là:
    $$P(\text{capture}) = 0.01 \times 0.001 = 0.00001 = 0.001\%$$
...
* **Cơ chế:**
  ...
  2. Collector giữ toàn bộ các Spans có cùng `traceId` trong một **In-memory Trace Buffer** trong khoảng thời gian chờ được định cấu hình (`decision_wait`, ví dụ 5s - 10s).
  3. Khi hết thời gian `decision_wait` hoặc nhận được Span kết thúc, Collector chạy tập luật chính sách (Sampling Policies):
     - Có bất kỳ Span nào mang cờ `Status = ERROR` hoặc `HTTP Status >= 500`? &rarr; **GIỮ 100%**.
     - Tổng thời gian `duration` của toàn bộ Trace có vượt quá ngưỡng 2000ms (p99 latency outlier)? &rarr; **GIỮ 100%**.
     - Trace có chứa attribute nghiệp vụ quan trọng (ví dụ `user.tier == "enterprise"`, `payment.amount > 10000`)? &rarr; **GIỮ 100%**.
...
* **Hậu quả:** 
  ...
  - `Collector Pod 1` chỉ thấy `Span A` (thành công) &rarr; quyết định **DROP**!
...
1. **Tier 1 (Agent / Router Tier):** Chạy dưới dạng DaemonSet trên từng Node hoặc stateless Gateway. Tier 1 không thực hiện tail-sampling mà sử dụng `loadbalancing` exporter:
   ...
   - Băm nhất quán (Consistent Hash Ring) theo công thức: `Collector_Index = hash(trace_id) % N`.
...
* **Bản chất:** Để đánh giá Tail-sampling, Collector phải giam giữ hàng chục nghìn Trace trong RAM suốt khoảng thời gian `decision_wait` (8s). 
...
* **Biện pháp phòng ngừa bắt buộc:**
  ...
  2. **Tính toán dung lượng RAM cho Collector:**
     ```text
     RAM cần thiết = (New Traces/sec × decision_wait × Average Trace Size) × 2.5 (Headroom)
     ```
     *Ví dụ:* 2,000 traces/s × 8s × 4 KB = 64 MB raw data. Khi cộng thêm overhead của Go runtime và metadata struct, bạn cần cấp phát tối thiểu **1.5GB - 2GB RAM limit** cho mỗi Collector Pod.
...
* **Kịch bản:**
  - Một user request kích hoạt một luồng xử lý bất đồng bộ (ví dụ: Spring `@Async`, Kafka Producer &rarr; Consumer, Saga Orchestrator).
  - Web service trả về `HTTP 202 Accepted` sau 50ms.
  - Worker xử lý ngầm (chuyển đổi video, sinh báo cáo PDF) chạy mất 25s rồi mới bị lỗi OutOfMemory hoặc Database Connection Timeout.
* **Hậu quả nếu cấu hình `decision_wait: 8s`:**
  - Sau 8 giây kể từ Span đầu tiên, Collector kiểm tra thấy Web service trả về 202 OK, không có lỗi gì &rarr; **Đưa ra quyết định DROP trace**!
...
* **Bối cảnh:** Một ngân hàng số triển khai Tail-based Sampling với `decision_wait: 15s` trên cụm Kubernetes gồm 4 Pods OTel Collector Contrib (mỗi Pod cấp 2GB RAM).
* **Sự cố:** Cụm cơ sở dữ liệu PostgreSQL Primary gặp sự cố deadlock, khiến hàng nghìn kết nối từ các service bị treo. Thay vì hoàn tất trong 200ms, toàn bộ các request đều kéo dài từ 10s đến 30s.
* **Hậu quả:** 
  - Vì các trace không kết thúc và `decision_wait` được đặt tới 15s, các Spans bị dồn ứ lại trong RAM của Collector với tốc độ khủng khiếp.
  - Collector không được cấu hình `memory_limiter`.
  - Cả 4 Pods OTel Collector lần lượt chạm ngưỡng 2GB RAM và bị Linux OOM-killer kết liễu (`Killed: 137`). Khi Pod khởi động lại, lượng Span tồn đọng ập vào tiếp tục khiến Pod crash-loop liên tục, làm tê liệt toàn bộ hạ tầng giám sát đúng vào thời điểm hệ thống đang gặp sự cố nghiêm trọng nhất!
* **Bài học rút ra:**
  1. Giảm `decision_wait` xuống mức tối ưu: 6s - 8s cho các synchronous API.
