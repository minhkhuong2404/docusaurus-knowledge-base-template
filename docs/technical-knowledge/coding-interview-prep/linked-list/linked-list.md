...
## Worked Example 4: Design LRU Cache (LeetCode #146)

**Problem**: Design a data structure that implements `get(key)` and `put(key, value)` with $\mathcal{O}(1)$ time complexity, evicting the least recently used item when reaching capacity.

**Approach**: Combine a `HashMap<Integer, Node>` (for $\mathcal{O}(1)$ lookup) with a **Doubly Linked List** (for $\mathcal{O}(1)$ node deletion and insertion at the head).
...
**Time**: $\mathcal{O}(1)$ for `get` and `put` | **Space**: $\mathcal{O}(\text{capacity})$
