...
---
import SpringDataRepositoryHierarchyDiagram from '@site/src/components/SpringDataRepositoryHierarchyDiagram';
import SpringDataPerformanceChecklistDiagram from '@site/src/components/SpringDataPerformanceChecklistDiagram';
...
<SpringDataRepositoryHierarchyDiagram />
...
// Naming convention: {RepositoryName}Impl — Spring auto-detects it
...
<SpringDataPerformanceChecklistDiagram />
...
> `@Version` (optimistic locking) is appropriate when conflicts are rare and you can afford to retry on conflict — it adds zero database lock overhead. `PESSIMISTIC_WRITE` (`SELECT FOR UPDATE`) is appropriate when conflicts are frequent and a retry would be expensive or unacceptable — for example, decrementing inventory where you must guarantee exclusivity. Optimistic is almost always preferred for web applications where most operations don't conflict.

**Q: Why does `GenerationType.IDENTITY` disable batch inserts?**

> With IDENTITY, the database assigns the primary key after the INSERT. Hibernate needs the ID immediately after each INSERT to populate the entity's `@Id` field, so it must execute and return from each INSERT one at a time — batch accumulation is impossible. `GenerationType.SEQUENCE` allows Hibernate to pre-allocate IDs from a sequence (in configurable chunks via `allocationSize`), so it knows the ID before executing the INSERT and can batch multiple statements into one JDBC batch.

**Q: What is the difference between `Specification` and `@Query`?**

> `@Query` is best when the query conditions are always the same — the JPQL or SQL is written once and reused. `Specification` (wrapping the JPA Criteria API) is best when some conditions are optional at runtime — it lets you compose individual predicate fragments dynamically at call time. Both are correct tools; the choice depends on whether the query is static or dynamic.

**Q: Why should `@Modifying` include `clearAutomatically = true`?**

> Bulk updates/deletes via `@Modifying` execute directly in the database and bypass Hibernate's first-level cache (persistence context). If entities affected by the bulk update are already loaded in the current session, their in-memory state is now stale — subsequent reads from the same transaction will return the pre-update values from cache, not the updated database values. `clearAutomatically = true` clears the persistence context after the bulk operation, forcing subsequent reads to re-query the database.
...
- [Spring Boot - Advanced Topics](./spring-boot-advanced.md)
- [Spring Framework: Deep Dive](./spring-framework-deep-dive.md)
- [Hibernate: Transactions and Performance](./hibernate-transactions-performance.md)
