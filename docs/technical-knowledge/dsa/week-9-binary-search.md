...
```mermaid
flowchart TD
    Start(["Input Problem"]) --> Q1{"Is input sorted or answers<br/>ordered / monotonic?"}
    Q1 -- "Yes" --> Q2{"Searching for a VALUE<br/>or OPTIMAL THRESHOLD?"}
    Q1 -- "No" --> Other["Consider Hash Table,<br/>Two Pointers, etc."]
    Q2 -- "Threshold" --> Q3{"Monotonic Property?<br/>(If X works, X+1 works too)"}
    Q3 -- "Yes" --> BSAns["Binary Search on Answer Space ✅"]
    Q3 -- "No" --> GreedyDP["Greedy or DP<br/>(Not binary search)"]
    Q2 -- "Value" --> Q4{"Is array perfectly sorted?"}
    Q4 -- "Yes" --> StdBS["Standard Binary Search ✅<br/>(Template 1 / 2)"]
    Q4 -- "No" --> Q5{"Is it ROTATED sorted?"}
    Q5 -- "Yes" --> RotBS["Rotated Array Binary Search ✅<br/>(Template 4)"]
    Q5 -- "No" --> MatSearch["2D Matrix Search<br/>(Row-by-row or staircase search)"]
```
