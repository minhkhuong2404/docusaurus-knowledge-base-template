...
interface SeqStep {
  label: string;
  sender: 'Client' | 'App' | 'Redis' | 'DB';
  receiver: 'Client' | 'App' | 'Redis' | 'DB';
  msg: string;
...
const STEPS: SeqStep[] = [
  ...
  { label: 'Step 2: Cache Query', sender: 'App', receiver: 'Redis', msg: 'Query cache:123', type: 'GENERAL', detail: 'Application checks if the data exists in Redis cache.' },
  { label: 'Step 3a: Cache Hit', sender: 'Redis', receiver: 'App', msg: 'Return cached payload', type: 'HIT', detail: 'Data found! Redis returns the value immediately. Steps 4-5 are bypassed.' },
  { label: 'Step 3b: Cache Miss', sender: 'Redis', receiver: 'App', msg: 'Null / Miss', type: 'MISS', detail: 'Data not in cache. Application must fall back to the database.' },
  { label: 'Step 4: DB Fetch', sender: 'App', receiver: 'DB', msg: 'Fetch record', type: 'MISS', detail: 'Application queries primary database or read replica for the record.' },
  { label: 'Step 5: Cache Set', sender: 'App', receiver: 'Redis', msg: 'Set key with TTL', type: 'MISS', detail: 'Application asynchronously writes retrieved data back to Redis with a TTL for future reads.' },
  { label: 'Step 6: Return Response', sender: 'App', receiver: 'Client', msg: 'Return record value', type: 'GENERAL', detail: 'Application returns the data to the client.' },
];

export default function CacheAsideSequenceDiagram(): React.JSX.Element {
  ...
  return (
    <div className="interactive-diagram-container" style={{ fontFamily: 'var(--ifm-font-family-base)' }}>
      <div className="interactive-diagram-header">
        ...
        <span style={{ color: '#34d399' }}>Cache-Aside (Lazy Loading) Sequence Player</span>
      ...
      <div className="aside-grid">
        ...
        <div className="interactive-diagram-svg-wrapper interactive-diagram-grid-bg">
          <svg viewBox="0 0 350 200" className="interactive-diagram-svg">
            <defs>
              <marker id="aside-arr" viewBox="0 0 10 10" refX="5" refY="5" markerWidth="5" markerHeight="5" orient="auto-start-reverse">
                <path d="M 0 1.5 L 8 5 L 0 8.5 z" fill="rgba(148,163,184,0.3)" />
              </marker>
              <marker id="aside-arr-color" viewBox="0 0 10 10" refX="5" refY="5" markerWidth="5" markerHeight="5" orient="auto-start-reverse">
            ...
            <text x="220" y="32" textAnchor="middle" fill="#fbbf24" fontSize="7.5" fontWeight="bold">Redis</text>
            ...
            {(() => {
              const xCoords = { Client: 40, App: 130, Redis: 220, DB: 310 };
                  ...
                  {STEPS.map((s, idx) => {
                    ...
                    return (
                      <g key={idx} style={{ opacity: 0.15 }}>
                        <line x1={sx} y1={sy} x2={ex > sx ? ex - 6 : ex + 6} y2={sy} stroke="rgba(255,255,255,0.3)" strokeWidth="1" markerEnd="url(#aside-arr)" />
