...
const STEPS: SeqStep[] = [
  ...
  { label: 'Step 2: Cache Write', sender: 'App', receiver: 'Cache', msg: 'Write to cache key', type: 'WRITE', detail: 'Application writes the payload directly into the in-memory cache.' },
  { label: 'Step 3: Instant ACK', sender: 'Cache', receiver: 'App', msg: 'Instant ACK (0ms)', type: 'GENERAL', detail: 'Cache acknowledges the write immediately. User is unblocked without waiting for database I/O.' },
  { label: 'Step 4: Return Response', sender: 'App', receiver: 'Client', msg: 'Return success', type: 'GENERAL', detail: 'Application returns transaction success status to the client at sub-millisecond speeds.' },
  { label: 'Step 5: Async Batch flush', sender: 'Cache', receiver: 'DB', msg: 'Asynchronous flush batch (v2)', type: 'ASYNC', detail: 'Later, a background daemon picks up the dirty keys and flushes them to the database in batches.' },
  { label: 'Step 6: DB Commit ACK', sender: 'DB', receiver: 'Cache', msg: 'Commit confirmation', type: 'GENERAL', detail: 'Database commits the batch update and clears dirty flags in Cache.' },
];

export default function WriteBehindSequenceDiagram(): React.JSX.Element {
  ...
  return (
    <div className="interactive-diagram-container" style={{ fontFamily: 'var(--ifm-font-family-base)' }}>
      <div className="interactive-diagram-header">
        ...
        <span style={{ color: '#34d399' }}>Write-Behind (Write-Back) Sequence Player</span>
      ...
      <div className="writebehind-grid">
        
        {/* SVG Viewport */}
        <div className="interactive-diagram-svg-wrapper interactive-diagram-grid-bg">
          <svg viewBox="0 0 350 200" className="interactive-diagram-svg">
            <defs>
              <marker id="wb-arr" viewBox="0 0 10 10" refX="5" refY="5" markerWidth="5" markerHeight="5" orient="auto-start-reverse">
                <path d="M 0 1.5 L 8 5 L 0 8.5 z" fill="rgba(148,163,184,0.3)" />
              </marker>
              <marker id="wb-arr-color" viewBox="0 0 10 10" refX="5" refY="5" markerWidth="5" markerHeight="5" orient="auto-start-reverse">
                <path d="M 0 1.5 L 8 5 L 0 8.5 z" fill={step.type === 'WRITE' ? '#ef4444' : step.type === 'ASYNC' ? '#fbbf24' : '#38bdf8'} />
              </marker>
            </defs>
              ...
              return (
                <g>
                  ...
                  {STEPS.map((s, idx) => {
                    ...
                    return (
                      <g key={idx} style={{ opacity: 0.15 }}>
                        <line x1={sx} y1={sy} x2={ex > sx ? ex - 6 : ex + 6} y2={sy} stroke="rgba(255,255,255,0.3)" strokeWidth="1" markerEnd="url(#wb-arr)" />
                  ...
                  <line
                    ...
                    y2={yVal}
                    stroke={step.type === 'WRITE' ? '#ef4444' : step.type === 'ASYNC' ? '#fbbf24' : '#38bdf8'}
                    strokeWidth="1.5"
                    className="interactive-diagram-flowing-path"
                    markerEnd="url(#wb-arr-color)"
                  />
                  <text
                    x={(startX + endX) / 2}
                    y={yVal - 4}
                    textAnchor="middle"
                    fill={step.type === 'WRITE' ? '#ef4444' : step.type === 'ASYNC' ? '#fbbf24' : '#38bdf8'}
                    fontSize="6.5"
                    fontWeight="bold"
                  >
                    {step.msg}
        ...
        <div className="interactive-diagram-details-card" style={{
          borderLeft: `4px solid ${step.type === 'WRITE' ? '#ef4444' : step.type === 'ASYNC' ? '#fbbf24' : '#38bdf8'}`,
          display: 'flex', flexDirection: 'column', gap: '6px', minHeight: '120px'
        }}>
          <div>
            <h4 style={{ margin: 0, fontSize: '11px', color: step.type === 'WRITE' ? '#ef4444' : step.type === 'ASYNC' ? '#fbbf24' : '#38bdf8' }}>
              {step.label}
            </h4>
          </div>
